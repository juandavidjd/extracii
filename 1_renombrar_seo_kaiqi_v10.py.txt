...
Cambios clave v10 vs v9:
- Mejora fuerte en "detección de nombre rico" para nombres tipo
  "1-11-131-PASTILLAS-DEL-AK-AKT125-FLEX-2011-VICTORY-ONE-100ST-CB150-2017-CB160F-2017".
  Ahora se consideran ricos y se respetan (NO se reemplazan por IA).
- IA SOLO se usa cuando el nombre es pobre y NO hay match en catálogos.
- Se sigue evitando marcar falsos duplicados: si el slug se repite, se versiona -v2, -v3, etc.
- Log detallado en C:\img\LOGS\log_renombrado_seo_v10.csv

Requisitos:
- Python 3.9+
- Paquetes: pandas, openpyxl, openai (nuevo cliente OpenAI), unicodedata
- Variable de entorno OPENAI_API_KEY configurada para usar Vision gpt-4o (opcional)
"""
...
LOG_CSV = os.path.join(LOG_DIR, "log_renombrado_seo_v10.csv")

# Parámetros de heurística
# Mínimo de tokens (palabras / códigos separados) para considerar un nombre "rico".
MIN_RICH_TOKENS = 5
...
def extraer_tokens_crudos(nombre_sin_ext: str) -> List[str]:
    """Extrae tokens alfanuméricos (separando guiones), ej:
    "1-11-131-PASTILLAS-DEL-AK-AKT125-FLEX-2011" ->
    ["1", "11", "131", "pastillas", "del", "ak", "akt125", "flex", "2011"]
    """
...
def es_nombre_rico(nombre_sin_ext: str) -> bool:
    ...
    Criterios (cualquiera de estos):
    - Mínimo de tokens
    - Contiene palabras de marca/modelo
    - Contiene varios tokens numéricos (años, cilindraje, códigos)
    """
    tokens = extraer_tokens_crudos(nombre_sin_ext)
    if len(tokens) >= 12:
        # nombre largo casi seguro rico
        return True
...
def limpiar_prefijo_codigo(nombre_sin_ext: str) -> Tuple[str, Optional[str]]:
    ...
    """
    # Separa por el primer guión fuerte numérico
...
def detectar_columnas_codigo(df: pd.DataFrame) -> List[str]:
    ...
    for c in df.columns:
        ...
        if any(k in cu for k in ["COD", "CÓD", "REF", "SKU", "REFERENCIA", "ITEM"]):
            cols.append(c)
    return cols
...
def construir_slug_desde_record(rec: Record) -> str:
    """Arma un slug base desde un record local (descripcion + marca/modelo/cilindraje)."""
    partes = [
        rec.get("descripcion", ""),
        rec.get("marca", ""),
        rec.get("modelo", ""),
...
def construir_slug_rico_desde_nombre(nombre_sin_ext: str) -> Tuple[str, str, str]:
    """Para nombres ricos tipo '1-11-131-PASTILLAS-DEL-AK-AKT125-...':
    - Quita prefijo numérico (1-11-131)
    - Devuelve slug limpio solo de la parte descriptiva
    - Devuelve también el texto descriptivo base y el prefijo encontrado (si hubiera)
    """
...
def main() -> None:
    print("\n============================================")
    print("  🔧 RENOMBRAR SEO KAIQI v10 (robusto)")
    print("============================================\n")
    ...
    for fname in files:
        ...
        if es_nombre_rico(nombre_sin_ext):
            ...
            # 2) Intentamos match en catálogos locales por código (INV > JC > YOKO > STORE > LEO > JAPAN > VAISAND)
            ...
            if rec is not None:
                ...
                ia_marca = ia_data.get("marca_moto", "")
                ia_modelo = ia_data.get("modelo_moto", "")
                ia_cilindraje = ia_data.get("cilindraje", "")
                ia_numeros = ia_data.get("numeros_molde", "")
