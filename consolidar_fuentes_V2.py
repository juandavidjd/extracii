...
# Borramos el CSV maestro anterior para evitar duplicados
...
for d in DIRECTORIOS:
    # Agregamos los CSV de la competencia (Japan, Leo, etc.)
    csv_files.extend(glob.glob(os.path.join(d, 'Base_Datos_*.csv')))
# Agregamos explícitamente el nuevo archivo de HTML
csv_files.append(os.path.join(PROJECT_DIR, 'Base_Datos_Fuentes_HTML.csv'))
...
for f in csv_files:
    if not os.path.exists(f):
        print(f"   -> ADVERTENCIA: No se encontró el archivo {f}. Saltando.")
        continue
        
    print(f"   -> Procesando {os.path.basename(f)}")
    try:
        df = pd.read_csv(f, encoding='utf-8-sig') # Probar con utf-8-sig primero
    except Exception as e1:
        try:
            df = pd.read_csv(f, encoding='latin-1') # Fallback a latin-1
        except Exception as e2:
            print(f"     ... Error leyendo {f}: {e1} / {e2}")
            continue
...
df_maestro.to_csv(OUTPUT_CSV, index=False, encoding='utf-8-sig')
...
if os.path.exists(html_fuentes_dir):
    for d in os.listdir(html_fuentes_dir):
        if (d.endswith('_files') or d.endswith('_files_')) and os.path.isdir(os.path.join(html_fuentes_dir, d)):
    ...
    for root, dirs, files in os.walk(folder):
        for img_file in files:
            ...
            if img_file.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
                ...
                # Usamos el nombre base para evitar conflictos de rutas
                dest_path = os.path.join(OUTPUT_IMG_DIR, os.path.basename(img_file))
