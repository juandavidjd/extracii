    ...
    if filename.endswith(".html") or filename.endswith(".htm"):
        ...
        try:
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                # --- LÓGICA DE EXTRACCIÓN (Mercado Libre V2) ---
                # 1. Encontrar todos los contenedores de producto
                items = soup.find_all('li', class_='ui-search-layout__item')
                if not items:
                    print(f"   -> {filename}: No se encontraron 'li' de producto. Saltando.")
                    continue

                for item in items:
                    # 2. Encontrar el Título (poly-component__title)
                    titulo_tag = item.find('a', class_='poly-component__title')
                    nombre = titulo_tag.get_text() if titulo_tag else None
                    
                    # 3. Encontrar la Imagen (poly-component__picture)
                    ...
                    if nombre and src:
                        ...
                        # Limpiar URL (quitar ?... y http...) y obtener solo el nombre de archivo
            ...
            print(f"   -> {filename}: Encontrados {len(items)} items.")
...
if productos_encontrados:
    ...
    df_externo.to_csv(OUTPUT_CSV, index=False, encoding='utf-8-sig')
    print("\n" + "="*40)
