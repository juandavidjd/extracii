...
# -*- coding: utf-8 -*-
"""
Premios Revancha (múltiples filas por sorteo) -> data\crudo\revancha_premios.csv
...
SELECTORES_TABLA = ["table", ".prizes table", ".tabla-premios table", ".results-table table"]
...
def main():
    ...
    if args.from_sorteo and args.to_sorteo:
        ...
        if start is None:
            log("[INFO] Sin base de sorteo. Usa --from/--to para backfill.")
    ...
    for s in sorteos:
        ...
        try:
            ...
            if premios:
                ...
                log(f"[WARN] No hallé tabla de premios en {s}")
