...
print("--- SCRIPT 3 (V4 - Corrección de Error de Precio) ---")
...
for index, row in df_kaiqi.iterrows():
    ...
    if row['Nivel_Confianza'] >= 85:
        desc_final = format_title(row['Descripcion_Rica']) 
    else:
    ...
    # --- AQUÍ LA CORRECCIÓN V4 ---
    # 1. Convertir a número
    price_num = pd.to_numeric(row['Precio'], errors='coerce')
    # 2. Si falla (NaN), poner 0. Si no, usar el número
    price_final = 0.0 if pd.isna(price_num) else float(price_num)
    # ---------------------------
    ...
    nueva_fila = {
        ...
        "Status": "active",
        "Variant SKU": row['SKU'],
        "Variant Price": price_final, # <-- Usar la variable corregida
        "Variant Grams": 500,
        "Variant Inventory Tracker": "shopify",
        "Variant Inventory Qty": 10,
        "Variant Inventory Policy": "continue" if price_final > 0 else "deny", # <-- Usar la variable corregida
...
print("\n" + "="*50)
print(f"✅ ¡PROYECTO FINALIZADO (V4)!")
