...
ALIAS_MAP = {
    ...
    # Nuevos alias para resolver pendientes y typos
...
PREFIJOS = {
    ...
    "BOMBILLOS": "ELE-BOM",           # NUEVO: Bombillos
    ...
    "EMPAQUES CARTER": "EMP-CAR",
    "SELLOS DOBLE RESORTE VERDES": "EMP-SEL", # NUEVO: Sellos
    "RIN": "RIN",
    "TAPA GASOLINA": "TAP-GAS",
    "VOLANTE": "MOT-VOL",
    "TENSOR CADENILLA": "MOT-TEN-CAD",
    "PIÑON SALIDA": "TRA-PIN-SAL",    # NUEVO: Piñón de salida
}
...
def base_system_from_category(cat_norm):
    ...
    if cat_norm in ("BOMBA ACEITE","FILTRO ACEITE","FILTRO CENTRIFUGO", "MANGUERA ACEITE"):
        return "Lubricación"
    ...
    if cat_norm in ("DISCOS CLUTCH", "PRENSA CLUTCH CON DISCOS"): # NUEVO
        return "Embrague"

    if cat_norm in ("KIT CARBURADOR","BAQUELA CARBURADOR","CONECTOR CARBURADOR","LLAVE GASOLINA", "CARBURADORES"): # NUEVO
        return "Carburación"

    if cat_norm in ("KIT EMPAQUES CTO","EMPAQUES TAPA VOLANTE","EMPAQUES TAPA CULATIN ORING","EMPAQUES ANILLO EXOSTO","EMPAQUES CONECTOR MOFLE","KIT RETENEDORES","KIT RETENEDORES MOTOR", "EMPAQUES CARTER", "SELLOS DOBLE RESORTE VERDES"): # NUEVO
...
def derive_subsystem_and_component(cat_norm, desc):
    ...
    if cat_norm in ("PIÑON SALIDA",): # NUEVO
        return "Transmisión secundaria", "Piñón de Salida"
    ...
    if cat_norm in ("CDI","REGULADOR","REGULADOR TRIFASICO","RELAY UNIVERSAL","FLASHER ELETRONICO", "BOMBILLOS"):
        if cat_norm == "BOMBILLOS":
            return "Iluminación", "Bombillos" # Subsystem/Component for bombillos
    ...
    if cat_norm == "MANGUERA ACEITE":
        return "Lubricación", "Manguera"
    ...
    if cat_norm == "PRENSA CLUTCH CON DISCOS": # NUEVO
        return "Embrague", "Prensa con Discos"
    ...
    if cat_norm == "CARBURADORES": # NUEVO
        return "Alimentación", "Carburador Completo"
    ...
    if cat_norm == "EMPAQUES CARTER":
        return "Empaques/Retenedores", "Carter"
    if cat_norm == "SELLOS DOBLE RESORTE VERDES": # NUEVO
        return "Empaques/Retenedores", "Sellos de Válvula/Eje"
