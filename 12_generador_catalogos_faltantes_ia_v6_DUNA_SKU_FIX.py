    ...
    if ruta_real:
        ...
        try:
            ...
            if col_archivo in df.columns:
                ...
                print(f"   ⚠️ Columnas no encontradas: {col_archivo} / {col_sku}")
...
def analizar_imagen_solo_clasificacion(ruta_img, nombre_ref):
    ...
    if not client: 
        # Respuesta default si no hay IA
...
LOTE_DUNA = {
    ...
    "referencia": "Base_Datos_Duna.csv", # El archivo v2 que renombramos
    "salida": os.path.join(BASE_DIR, "catalogo_kaiqi_imagenes_DUNA.csv"),
    
    # Columnas Clave del CSV Duna v2
    "col_archivo": "Imagen_SEO", 
    "col_nombre": "Nombre_Producto",
    "col_sku": "Referencia_Real"  # <--- ¡ESTA ES LA CLAVE!
}
...
def procesar_duna_fix():
    ...
    # Cargar referencias completas
    refs = cargar_referencias(LOTE_DUNA['referencia'], LOTE_DUNA['col_archivo'], LOTE_DUNA['col_nombre'], LOTE_DUNA['col_sku'])
    ...
    def worker(img_file):
        ...
        if datos_maestros:
            ...
            # Fallback si la imagen no cruza con el CSV (raro si vienen del scraper)
            nombre_final = os.path.splitext(img_file)[0].replace("-", " ").title()

        # 2. Usar IA SOLO para clasificar (Sistema, Subsistema)
        # Le pasamos el nombre real para que sepa qué clasificar
        ia = analizar_imagen_solo_clasificacion(ruta_completa, nombre_final)
        
        # 3. Construir Fila (Respetando Nombre y SKU)
        return {
            ...
            "Confianza_Global": 1.0, # Confianza total porque viene de scraping directo
